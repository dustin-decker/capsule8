...
func (monitor *EventMonitor) resolveSymbol(bin, symbol string) (string, error) {
	...
	// Find the base address for all object types other than shared objects
	...
	for _, sym := range symbols {
		if sym.Name == symbol {
			return fmt.Sprintf("%#x", sym.Value-baseAddress), nil
		}
	}
	...
	return "", fmt.Errorf("Symbol %q not found in %q", symbol, bin)
}
